...
> **참고:** 이들 환경은 서드파티 도구이며, 업데이트 주기는 각 도구의 관리자가 정하므로 AIdea 플러그인의 릴리스 일정과 무관합니다. 제공자 카드의 "환경 설치/업데이트"는 해당 제공자의 CLI(Codex CLI, Gemini CLI)를 설치하고 업데이트합니다. 번역 탭의 "환경 설치"는 전체 문서 번역에 필요한 Python 환경과 `pdf2zh_next`를 설치하고 업데이트하며, 이때 인터넷 연결이 필요합니다. 업스트림 도구가 업데이트되면 해당 버튼을 다시 실행하면 되고, 플러그인 업데이트를 기다릴 필요가 없습니다. 플러그인을 업데이트해도 이들 환경이 자동으로 갱신되지는 않습니다.
