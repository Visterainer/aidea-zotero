...
- **選択範囲の翻訳** を PDF リーダーの選択ポップアップで実行し、ローカルのコールドスタートキャッシュを再利用して Zotero ノートへ追加可能
...
選択範囲の翻訳を使う場合は、設定で機能を有効にしてモデルを選び、PDF リーダーでテキストを選択します。論文ごとの初回利用時には、AIdea が要約と重要用語を含むローカルのコールドスタートキャッシュを作成し、以後の翻訳で文脈として再利用します。
