...
使用你的**已有账号**通过 OAuth 登录 —— 无需 API Key，也不需要 API 平台的订阅。插件支持多个服务商，采用不同的 OAuth 流程实现无缝认证。

> **最新支持版本：ChatGPT 5.4**

### 🌐 多服务商支持
- **OpenAI (ChatGPT)** — 通过 Codex CLI OAuth 授权（需要 Node.js）
- **Google Gemini** — 插件内 OAuth 授权（Authorization Code + PKCE，需安装 Gemini CLI）
- **Qwen (通义千问)** — 插件内 OAuth 授权（Device Code 流程，无需额外安装）
- **GitHub Copilot** — 插件内 OAuth 授权（Device Code 流程，无需额外安装）
...
### 环境要求
- **Zotero 7 / 8**（7.0+ 版本）
- **Node.js**（v18+）—— OpenAI Codex 和 Gemini CLI 工具所需（通义千问和 GitHub Copilot 无需 Node.js）
...
### 3. OAuth 登录
在任意服务商卡片中点击**「OAuth 登录」**开始授权：
- **OpenAI / Gemini**：浏览器将打开进行身份验证，使用你的账号登录即可
- **通义千问 / GitHub Copilot**：弹窗显示授权码，点击确定自动复制授权码并打开浏览器，在页面中粘贴授权码完成授权

登录成功后，返回 Zotero 并点击**「刷新模型」**加载可用模型。
