...
除了 OAuth 登录外，AIdea 还支持连接任意 **OpenAI 兼容聊天端点**。这适合希望使用本地或自托管模型，或使用内置服务商以外服务的进阶用户。

如需使用自定义端点，请打开 **设置**，在连接模式选择器中找到 **Custom** 选项。你需要填写以下字段：

| 字段             | 必填 | 说明                                                                                                           |
| ---------------- | ---- | -------------------------------------------------------------------------------------------------------------- |
| **API Base URL** | 是   | 端点的基地址（例如 `https://api.openai.com/v1`，或本地地址如 `http://localhost:8000`）。支持本地及自托管端点。 |
| **API Key**      | 否   | 如果端点需要认证则填写 API Key；如无需认证可留空。                                                             |
| **Model**        | 是   | 要使用的模型名称（例如 `gpt-4o`、`llama3` 等）。                                                               |

> **注意：** 此功能面向 OpenAI 兼容的聊天端点，不保证对工具调用、文件上传或图片输入等服务商特定功能的兼容性。
