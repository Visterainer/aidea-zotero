...
### 🔐 ChatGPT 账号登录（无需 API Key）
使用你的 **ChatGPT 账号**通过 OAuth 登录 —— 无需 API Key，也不需要 API 平台的订阅。只要你有免费或付费的 ChatGPT 账号即可使用。插件借助 Codex CLI 的 OAuth 流程实现无缝认证。
...
### 3. OAuth 登录
在 OpenAI 卡片中点击**「OAuth 登录」**，浏览器将打开进行身份验证 —— **使用你的 ChatGPT 账号登录即可**（无需 API Key）。登录成功后，返回 Zotero 并点击**「刷新模型」**加载可用模型。
